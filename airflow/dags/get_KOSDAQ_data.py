...
get_kospi_data = get_priceData("get_KOSPI_DATA","192.168.90.128:1212/stock-price/kosdaq-all/day='{{ next_execution_date.strftime('%Y-%m-%d') }}'")
...
check_done_flag = BashOperator(
    ...
    bash_command="""
        RETURN_VALUE=$(curl 192.168.90.128:1212/check/hdfs/kosdaq-minute/{{next_execution_date.strftime('%Y-%m-%d')}})
...
load_hdfs_data = BashOperator(
    task_id = 'load_hdfs',
    bash_command = "curl 192.168.90.128:1212/hdfs/kosdaq-minute/{{next_execution_date}}",
    dag = dag)
